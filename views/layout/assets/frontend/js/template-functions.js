...
$( document ).ready( function(){
	...
	*/
	...
	// Google Map
	var mapIcon = [ assetsPath + 'images/marker/marker.png'];
	var mapLocations = [[-16.510549777156246,-68.12335044113502]];
	var mapInfoContent = ['La Paz, Bolivia<br>Avenida Arce esquina Pinilla<br>Edif. Arcadia #2588 Mezzanine oficina 106'];
		...
		contactForm: function(){
			...
			$('#form-contact-info').submit(function(event){
				...
				$.ajax({
					...
					/*beforeSend: function(){
						if(textComment.length == 0)
							{
								toastr.error('Debes escribir un comentario, por favor escribe algo!', '=( Comentario Invalido!');
								return false;
							}
					},*/
					success: function(resultado){
						if(resultado == 'false')
							{
								alert('El Correo Electrónico NO ha sido enviado correctamente!.');
								return false;
							}
				...
				/*$('#frm-comment').submit(function(e){
					var idContent = $('input#codcontent').val();
					var textComment = $('textarea#textarea-content-publication').val();
					var data = new FormData(this);
					$.ajax({
						url: globalURLEnlacemeConnection + 'content/commentContentView/'+idContent,
						data: data,
						processData: false,
						contentType: false,
						type: 'POST',
						...
						success: function(resultado){
							if(resultado == 'register')
								{
									toastr.warning('Lo sentimos muchisimo, debes estar registrado para Comentar esta publicación!', 'Registro Necesario Enlaceme.com');
								}
							else
								{				
								if(resultado == 'false')
									{
										toastr.error('Lo Sentimos Muchísimo, existe un problema al publicar su comentario, por favor intente más tarde.', 'Comentarios Enlaceme.com!');
										return false;
									}
								else
									{
										toastr.success('Tú comentario se ha registrado correctamente!.', 'Comentarios Enlaceme.com!');
										window.location = globalURLEnlacemeConnection + 'content/contentView/'+idContent;
										return true;
									}
								}
						}
